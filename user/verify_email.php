...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $query = db_prepare($conn, "find_user_by_code", 'SELECT id, email, fullname FROM users WHERE verification_code = ?');
    $result = db_execute($conn, "find_user_by_code", array($verificationCode));
    ...
    $checkExpiry = db_prepare($conn, "check_code_expiry", 'SELECT verification_code_expiry FROM users WHERE id = ?');
    $expiryResult = db_execute($conn, "check_code_expiry", array($userId));
    $expiryRow = db_fetch_assoc($expiryResult);
    ...
    $updateQuery = db_prepare($conn, "verify_user", 
        'UPDATE users SET is_verified = true, email_verified_at = NOW(), verification_code = NULL, verification_code_expiry = NULL WHERE id = ?');
    $updateResult = db_execute($conn, "verify_user", array($userId));
