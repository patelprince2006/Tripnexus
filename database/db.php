...
// Ensure train_stations table exists and has sample data
$check_train_stations_table = mysqli_query($conn, "SHOW TABLES LIKE 'train_stations'");
if (mysqli_num_rows($check_train_stations_table) == 0) {
    mysqli_query($conn, "CREATE TABLE IF NOT EXISTS train_stations (
    ...
    foreach ($sample_train_stations as $station) {
        db_query($conn, "INSERT IGNORE INTO train_stations (station_code, station_name, city, state) VALUES (?, ?, ?, ?)", $station);
    }
}

// Ensure bus_locations table exists and has sample data
$check_bus_locations_table = mysqli_query($conn, "SHOW TABLES LIKE 'bus_locations'");
if (mysqli_num_rows($check_bus_locations_table) == 0) {
    mysqli_query($conn, "CREATE TABLE IF NOT EXISTS bus_locations (
        location_id INT AUTO_INCREMENT PRIMARY KEY,
        city_name VARCHAR(100) NOT NULL UNIQUE,
        state VARCHAR(50) DEFAULT NULL
    ) ENGINE=InnoDB");
    
    $sample_bus_locations = [
        ['Bangalore', 'Karnataka'],
        ['Hyderabad', 'Telangana'],
        ['Mumbai', 'Maharashtra'],
        ['Pune', 'Maharashtra'],
    ...
    foreach ($sample_bus_locations as $location) {
        db_query($conn, "INSERT IGNORE INTO bus_locations (city_name, state) VALUES (?, ?)", $location);
    }
}
